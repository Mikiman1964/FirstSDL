...
BIS JETZT GEHT ES GANZ GUT VON DER HAND.  BIS DENN ERSTMAL     9  8  7  6  5  4  3  2  1  0                                                    "};
    ...
    if (InitScroller(&Scroller1,2,332,szMessage1,0.3,0.02,100,1,true,false) != 0) {
        return -1;
    }
    uScroller2Timer = SDL_GetTicks();
    if (InitScroller(&Scroller2,1,364,szMessage2,0,0,0,1,false,false) != 0) {
        return -1;
    }
    uScroller3Timer = SDL_GetTicks();
    if (InitScroller(&Scroller3,2,332,szMessage3,0,0,0,1,false,false) != 0) {
        return -1;
    }
    ...
    while (bRun) {
        ...
        fSin3 = sin(fAngle3) * 340;
        Scroller3.nYpos = 300 + (int)fSin3;
        ...
        // Ballons anzeigen
        DestR_Ballon.x = 430 - nBallonSize / 2;    // Position muss abhängig von Größe
        DestR_Ballon.y = 330 - nBallonSize / 2;    // sein, damit mittig angezeigt wird
        ...
        if (bShowFDLogo) {
            ...
            DestR_FDLogo.x = 380 - fFDLogoSizeFactor * 15;    // Position muss abhängig von Größe
